...
//add Recourse fucntion
    function addRecourse($module, $nature, $note_affiche, $note_reel, $fn, $ln, $g){
        //:fn<-marqueur nomme
        $query = $this->db->prepare("INSERT INTO recours (module, nature, note_affiche, note_reel, id_student) 
        VALUES (:m, :n, :na, :nr, (SELECT id FROM students WHERE nom=:fn AND prenom=:ln AND groupe=:g))");
...
//update student infos function
    function updateStudent($id){
        ...
        $query->bindParam(':nom', $firstname);
        $query->bindParam(':prenom', $lastname);
        $query->bindParam(':email', $email);
        $query->bindParam(':groupe', $group);
        $query->bindParam(':id', $id, PDO::PARAM_INT);
        $query->execute();
